...
test('代理安全配置使用精确来源匹配', async () => {
  ...
  assert.equal(normalizeOrigin('file:///Applications/MySSH.app/Contents/Resources/index.html'), 'file://')
  ...
  assert.equal(isRequestAllowed({ origin: 'file://', allowedOrigins: ['null', 'file://'], accessToken: 'secret', requestToken: 'secret' }), true)
  assert.equal(isRequestAllowed({ origin: 'file://', allowedOrigins: ['null', 'file://'], accessToken: '', requestToken: '' }), false)
